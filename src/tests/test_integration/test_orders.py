...
def test_create_stripe_session():
    ...
    stripe_session = create_stripe_session(
        total_amount=total_amount, titles="DieHard", message=message
    )
    ...
    assert stripe_session.success_url == settings.PAYMENT_SUCCESS_URL
    assert stripe_session.cancel_url == settings.PAYMENT_CANCEL_URL
