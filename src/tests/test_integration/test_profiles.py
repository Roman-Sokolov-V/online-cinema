...
async def test_create_user_profile_with_fake_s3(
    ...
    assert profile_data[
               "date_of_birth"] == "1990-01-01", "Date of birth does not match."
    ...
    assert str(
        profile_in_db.date_of_birth) == "1990-01-01", "Date of birth is incorrect!"
...
async def test_create_user_profile_invalid_auth(client, headers,
    ...
    assert response.json()[
               "detail"] == expected_detail, f"Unexpected error message: {response.json()['detail']}"
...
async def test_admin_creates_user_profile(
    ...
    5. Verify that the profile was created in the database.
    """
    admin_user = UserModel.create(email="admin@mate.com",
                                  raw_password="AdminPass123!", group_id=3)
    admin_user.is_active = True
    db_session.add(admin_user)

    regular_user = UserModel.create(email="user@mate.com",
                                    raw_password="UserPass123!", group_id=1)
...
async def test_user_cannot_create_another_user_profile(
    ...
    """
    user_1 = UserModel.create(email="user1@mate.com",
                              raw_password="User1Pass123!",
                              group_id=1)  # 1 = User
    user_1.is_active = True
    db_session.add(user_1)

    user_2 = UserModel.create(email="user2@mate.com",
    ...
    stmt_profile = select(UserProfileModel).where(
        UserProfileModel.user_id == user_2.id)
    result_profile = await db_session.execute(stmt_profile)
    profile_in_db = result_profile.scalars().first()
...
async def test_inactive_user_cannot_create_profile(
    ...
    """
    user = UserModel.create(email="inactive@mate.com",
                            raw_password="TestPassword123!", group_id=1)
...
async def test_profile_creation_fails_on_s3_upload_error(
    ...
    with patch.object(s3_storage_fake, "upload_file",
...
async def test_profile_creation_invalid_name(client, jwt_manager, first_name,
    ...
    assert expected_error in str(
        response.json()), f"Unexpected error message: {response.json()}"
...
@pytest.mark.asyncio
@pytest.mark.unit
async def test_profile_creation_avatar_too_large(db_session, client,
                                                 jwt_manager):
    """
...
async def test_profile_creation_invalid_birth_date(client, jwt_manager,
...
@pytest.mark.asyncio
async def test_update_one_field_user_profile_with_fake_s3(
        db_session,
        seed_user_groups,
        reset_db,
        jwt_manager,
    ...
    Steps:
    1. Create a test user and activate them.
    2. Try update one field by update_profile endpoint
    3. Check that this field successfully updated
    4. Check that All other fields have not changed their condition in the database
    """
    user, headers, profile = create_user_and_profile
    profile_url = f"/api/v1/profiles/users/{user.id}/profile/"
    ...
    update_files = {
        "first_name": (None, "Johnson"),
    }
    response = await client.patch(profile_url, headers=headers,
                                  files=update_files)
    await db_session.refresh(profile)
    assert response.status_code == 200
    assert profile.first_name == update_files["first_name"][
        1].lower(), "updated text-fields should be save in db in lower register"
    assert response.json()["first_name"] == update_files["first_name"][
        1].lower(), "response text data should be in lower register"
    profile_dict_after_updating = {
        column.name: getattr(profile, column.name) for column in
        profile.__table__.columns
    }
    profile_dict_before_updating.pop("first_name")
    profile_dict_after_updating.pop("first_name")
    assert profile_dict_after_updating == profile_dict_before_updating, "All other fields should not change their condition in the db"
...
async def test_update_all_fields_user_profile_with_fake_s3(
    ...
    Steps:
    1. Create a test user and activate them.
    2. Try update all fields by update_profile endpoint
    3. Check that this field successfully updated
    """
    user, headers, profile = create_user_and_profile
    profile_url = f"/api/v1/profiles/users/{user.id}/profile/"
    ...
    assert str(profile.date_of_birth) == "1991-02-02", "Date of birth is incorrect!"
