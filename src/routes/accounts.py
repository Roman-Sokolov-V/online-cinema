...
BASE_LINK = f"{settings.HOSTING}/api/v1/accounts/"
...
@router.post(
    ...
    response_model=UserRegistrationResponseSchema,
    summary="Request to send a new activation token.",
    description="Create a new activation token and send new activation email.",
    status_code=status.HTTP_201_CREATED,
    responses={
        409: {
            "description": "Conflict - User with this email already activate.",
            "content": {
                "application/json": {
                    "example": {
                        "detail": "A user with this email test@example.com already activate."
                    }
                }
            },
        },
        400: {
            "description": ("Bad Request - The provided email or password"),
            "content": {
                "application/json": {
...
async def send_new_activation_token(
    ...
    if not existing_user or not existing_user.verify_password(
        user_data.password
    ):
        raise HTTPException(
...
@router.post(
    ...
    description=(
        "Activate a user's account using one of two methods:\n\n"
        "1. **Self-activation**: Provide user's email and activation token in "
        "the request body.\n"
        "2. **Admin activation**: Provide user's email in the request body, "
        "and include a valid admin access token in the `Authorization` "
        "header.\n\n"
        "**Note:** In both cases, a successful response confirms that the "
    ...
    responses={
        ...
        403: {
            "description": "Forbidden - The provided access token does not belong to an admin user.",
            "content": {
                "application/json": {
                    "examples": {
                        "not_admin": {
                            "summary": "Not an Admin",
                            "value": {"detail": "User is not admin"},
                        }
                    }
                }
            },
...
async def activate_account(
    ...
    login_link = BASE_LINK + "login/"

    await email_sender.send_activation_complete_email(
        str(activation_data.email), login_link
    )

    return MessageResponseSchema(detail="User account activated successfully.")
...
async def request_password_reset_token(
    ...
    If the user exists and is active, invalidates any existing password reset
    tokens and generates a new one.
    ...
    password_reset_complete_link = BASE_LINK + "password-reset-complete/"
...
async def login_user(
    ...
    jwt_access_token = jwt_manager.create_access_token(
        {"user_id": user.id, "group": user.group.name}
    )
...
async def refresh_access_token(
    ...
    stmt: Any = select(RefreshTokenModel).filter_by(
...
async def logout(
    ...
    await db.execute(
        delete(RefreshTokenModel).where(
            RefreshTokenModel.user_id == token_payload["user_id"]
        )
    )
    await db.commit()
...
@router.post(
    "/change-password/",
    response_model=MessageResponseSchema,
    summary="Request to reset the user's password by providing current password",
    description="Change a user's password if user`s email, new and current passwords are provided.",
    status_code=status.HTTP_200_OK,
    responses={
        400: {
            "description": (
                "Bad Request - The provided email or current password is invalid."
            ),
            "content": {
                "application/json": {
                    "examples": {
...
async def change_password(
    ...
    Validates the token and updates the user's password if current password is valid.

    Args:
        data (PasswordChangeRequestSchema): The request data containing the user's email,
    ...
    if (
        ...
        or not user.verify_password(data.current_password)
    ):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def change_user_group(
    ...
    stmt = select(UserGroupModel.id).where(
        UserGroupModel.name == data.group_name
    )
    result = await db.execute(stmt)
    ...
    return MessageResponseSchema(
        detail="User`s group is changed successfully"
    )
