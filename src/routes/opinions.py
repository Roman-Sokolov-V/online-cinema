...
from database import get_db, MovieModel, UserModel, CommentModel, RateModel
...
@router.post(
    ...
    responses={
        ...
        400: {
            ...
            "content": {
                "application/json": {
                    "example": {"detail": "Movie already in favorite list."}
                }
            },
        },
    },
...
@router.delete(
    ...
    responses={
        ...
        400: {
            ...
            "content": {
                "application/json": {
                    "example": {"detail": "Movie not found in favorite list."}
                }
            },
        },
    },
...
async def remove_from_favorite(
    ...
    if not user:
        raise HTTPException(
            status_code=404, detail="User with the access token was not found."
        )
...
async def get_favorites(
    ...
    stmt = (
        select(MovieModel)
        .where(MovieModel.users_like.any(UserModel.id == user.id))
        .options(
...
@router.post(
    ...
    responses={
        ...
        400: {
            ...
            "content": {
                "application/json": {
                    "example": {"detail": "You already commented this movie"}
                }
            },
        },
    },
...
async def add_comment_to_movie(
    ...
    stmt = select(CommentModel.id).where(
        (CommentModel.user_id == user_id) & (CommentModel.movie_id == movie_id)
    )
    result = await db.execute(stmt)
    ...
    comment = CommentModel(
        content=comment_data.content, user_id=user_id, movie_id=movie_id
    )
    db.add(comment)
    await db.commit()
...
@router.post(
    ...
    responses={
        ...
        400: {
            ...
            "content": {
                "application/json": {
                    "example": {"detail": "You can't reply your commentary."}
                }
            },
        },
        404: {
...
async def add_reply_to_comment(
    ...
    return ResponseReplySchema.model_validate(reply, from_attributes=True)
...
@router.post(
    ...
    responses={
        ...
        400: {
            ...
            "content": {
                "application/json": {
                    "example": {"detail": "Movie already rated."}
                }
            },
        },
        404: {
...
async def rate_movie(
    ...
    stmt = select(RateModel).where(
        (RateModel.user_id == user_id) & (RateModel.movie_id == movie_id)
    )
    result = await db.execute(stmt)
